...
computed. The two-tailed spin p-value is:

$$p_{\text{spin}} = \frac{1}{1000}\left|\{p : |r_p| \geq |r_{\text{obs}}|\}\right|$$
...
randomisation (BrainSpace `moran`, 100 permutations, procedure `"singleton"`).
Eigenvectors of the spatial weight matrix are used to generate null maps with
autocorrelation matched to the observed data. The `"singleton"` procedure
produces a tighter match at the cost of a smaller number of feasible
randomisations.
